...
Class Config extends \Phalcon\Config
{
    ...
    {
        $this->_app->getDi()->setShared(
            ServiceMeta::SERVICE_MODELS_MANAGER,
            function () {
    ...
    protected function _extract($key, $module_name, $simple_xml_node)
    {
        switch ($key) {
            ...
            case "routes":
                $routes = array();
                if (!isset($simple_xml_node->routes) || !sizeof($simple_xml_node->routes->children())) {
                    return $routes;
                }
            ...
            case "observers":
                $observers = array();
                if (!isset($simple_xml_node->observers) || !sizeof($simple_xml_node->observers->children())) {
                    return $observers;
                }
