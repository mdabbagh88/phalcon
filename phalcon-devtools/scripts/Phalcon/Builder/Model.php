...
class Model extends Component
{
    ...
    public function __construct($options)
    {
        ...
        if (!isset($options["uri"])) {
            throw new BuilderException("Please, specify the model uri in the form module/class_name"); 
        }
        ...
        $options["module"]    = Utils::camelize($uri_parts[0]);
        $options["classParts"]= explode(" ", ucwords(str_Replace("_", " ", $uri_parts[1])));
        $options["className"] = end($options["classParts"]);
...
";
        ...
        $model_namespace             = \Cloud::app()->getModuleEntityNamespace(\Cloud\Core\Model\App::MVC_ENTITY_MODEL, $this->_options["module"]);
        for($i = 0; $i < sizeof($this->_options["classParts"]) - 1; $i++) {
            $model_namespace .= "\\{$this->_options["classParts"][$i]}";
        }
        $this->_options["namespace"] = $model_namespace;
        
        $model_dir         = \Cloud::app()->getModuleEntityDir(\Cloud\Core\Model\App::MVC_ENTITY_MODEL, $this->_options["module"]);
        
        $modelPath         = $model_dir;
        for($i = 0; $i < sizeof($this->_options["classParts"]) - 1; $i++) {
            $modelPath .= DS . $this->_options["classParts"][$i];
        }
        $modelPath        .= DS; 
